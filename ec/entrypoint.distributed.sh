...
./platform-admin.sh add-hosts --hosts $CONTROLLER_HOST --credential cred1
./platform-admin.sh submit-job --service controller --job install --args controllerPrimaryHost=$CONTROLLER_HOST controllerAdminUsername=admin controllerAdminPassword=admin controllerRootUserPassword=root newDatabaseRootPassword=root newDatabaseUserPassword=admin controllerProfile=Demo
...
scp -o StrictHostKeyChecking=no /home/appdynamics/license.lic $CONTROLLER_HOST:/home/appdynamics/platform/product/controller/license.lic

curl --user root@system:root -X POST http://appd-controller:8090/controller/rest/configuration?name=eum.cloud.host\&value=http://appd-eum:7001
curl --user root@system:root -X POST http://appd-controller:8090/controller/rest/configuration?name=eum.beacon.host\&value=http://localhost:7001
curl --user root@system:root -X POST http://appd-controller:8090/controller/rest/configuration?name=eum.beacon.https.host\&value=https://localhost:7002
curl --user root@system:root -X POST http://appd-controller:8090/controller/rest/configuration?name=eum.mobile.screenshot.host\&value=http://localhost:7001
curl --user root@system:root -X POST http://appd-controller:8090/controller/rest/configuration?name=eum.es.host\&value=http://appd-loadbalancer:9080
curl --user root@system:root -X POST http://appd-controller:8090/controller/rest/configuration?name=appdynamics.on.premise.event.service.url\&value=http://appd-loadbalancer:9080
curl --user root@system:root -X POST http://appd-controller:8090/controller/rest/configuration?name=analytics.agentless.event.service.url\&value=http://localhost:9080

ES_KEY=`curl --user root@system:root http://appd-controller:8090/controller/rest/configuration?name=appdynamics.es.eum.key | grep value |  sed "s@\s*<value>\(.*\)</value>\s*@\1@"`
echo "ES_KEY=$ES_KEY"
